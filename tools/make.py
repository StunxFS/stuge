...
SRC_MODIFIED = os.path.getmtime("src") != MAKEFILE_MTIME
THIRDPARTY_MODIFIED = os.path.getmtime("thirdparty") != MAKEFILE_MTIME

USE_CLEAN = "clean" if SRC_MODIFIED or THIRDPARTY_MODIFIED else ""

if NOT_EXISTS_MAKEFILE or ASSETS_MODIFIED or SRC_MODIFIED or THIRDPARTY_MODIFIED:
    with open("Makefile", "w") as mf:
        ...
        for i, cfile in enumerate(CFILES):
            objfile = OBJS[i]
            mf.write(f"{objfile}:\n")
            mf.write(f"	$(CC) {INCS} -Werror -c -o {objfile} {cfile}\n\n")
